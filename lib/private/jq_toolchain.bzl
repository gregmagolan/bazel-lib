...
JQ_PLATFORMS = {
    "darwin_amd64": struct(
        release_platform_name = "osx-amd64",
        compatible_with = [
            "@platforms//os:macos",
...
def _jq_platform_repo_impl(repository_ctx):
    ...
    release_platform_name = meta.release_platform_name if hasattr(meta, "release_platform_name") else repository_ctx.attr.platform
    jq_bin = "jq.exe" if is_windows else "jq"
    ...
    repository_ctx.download(
        ...
        output = jq_bin,
        executable = True,
        integrity = JQ_VERSIONS[repository_ctx.attr.version][release_platform_name],
    )

    build_content = """# Generated by @aspect_bazel_lib//lib/private:jq_toolchain.bzl
load("@aspect_bazel_lib//lib/private:jq_toolchain.bzl", "jq_toolchain")
exports_files(["{jq_bin}"])
jq_toolchain(name = "jq_toolchain", bin = "{jq_bin}", visibility = ["//visibility:public"])
""".format(jq_bin = jq_bin)

    # Base BUILD file for this repository
    repository_ctx.file("BUILD.bazel", build_content)

    # index.bzl file for this repository
    repository_ctx.file("defs.bzl", content = """# Generated by @aspect_bazel_lib//lib/private:jq_toolchain.bzl
platform = "{platform}"
version = "{version}"
def bin():
    "Returns the label of the jq binary for this platform"
    return Label("@{name}//:{jq_bin}")
""".format(
