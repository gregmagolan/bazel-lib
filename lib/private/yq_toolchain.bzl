...
def _yq_platform_repo_impl(repository_ctx):
    ...
    yq_bin = "yq.exe" if is_windows else "yq"
    ...
    repository_ctx.download(
        ...
        executable = True,
        integrity = YQ_VERSIONS[repository_ctx.attr.version][release_platform_name],
    )

    build_content = """# Generated by @aspect_bazel_lib//lib/private:yq_toolchain.bzl
load("@aspect_bazel_lib//lib/private:yq_toolchain.bzl", "yq_toolchain")
exports_files(["{yq_bin}"])
yq_toolchain(name = "yq_toolchain", bin = "{yq_bin}", visibility = ["//visibility:public"])
""".format(yq_bin = yq_bin)

    # Base BUILD file for this repository
    repository_ctx.file("BUILD.bazel", build_content)

    # index.bzl file for this repository
    repository_ctx.file("defs.bzl", content = """# Generated by @aspect_bazel_lib//lib/private:yq_toolchain.bzl
platform = "{platform}"
version = "{version}"
def bin():
    "Returns the label of the yq binary for this platform"
    return Label("@{name}//:{yq_bin}")
""".format(
